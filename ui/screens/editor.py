...
def render(db: JSONStore):
    ...
    st.text_area("Initial text", key="input_text", height=200,
                 placeholder="Paste or type initial copy…")
    st.text_area(
    ...
    cols = st.columns(2)
    with cols[0]:
        length = st.selectbox("Copy length", options=[
                              "short", "medium", "long"], index=1)
    with cols[1]:
        strength = st.select_slider("Tone strength", options=[
                                    "subtle", "balanced", "strong"], value="balanced")
    tov_text = st.session_state.get("tov_text") or db.get_tone_of_voice()
    st.text_area("Tone of Voice (applied)", value=tov_text,
                 key="tov_preview", height=220, disabled=True)
    ...
    def do_generate():
        ...
        prompt = build_prompt(tov, st.session_state.get(
            "input_text", ""), st.session_state.get("custom_instructions", ""), params)
        ...
        db.append_history_item(
            st.session_state["project"], st.session_state["session_id"], item)

    st.button("Generate", type="primary", on_click=do_generate, disabled=not (
        st.session_state.get("input_text") or st.session_state.get("custom_instructions")))

    last = st.session_state.get("last_generation")
    if last:
        st.markdown("---")
        st.markdown("**Output**")
        st.session_state["output_area"] = st.session_state.get(
            "output_area", last["output"]) or last["output"]
        ...
        def do_regenerate():
            ...
            params = GenerationParams(length=length, strength=strength)
            prompt = build_prompt(db.get_tone_of_voice(),
                                  last["input"], last["instructions"], params)
            out = engine.generate(prompt, params)
            st.session_state["output_area"] = out
            item = {**last, "output": out,
                    "params": {"length": length, "strength": strength}}
            st.session_state["history"].append(item)
            st.session_state["last_generation"] = item
            db.append_history_item(
        ...
        copy_to_clipboard_button(st.session_state.get(
            "output_area", last["output"]), key="out_copy")
